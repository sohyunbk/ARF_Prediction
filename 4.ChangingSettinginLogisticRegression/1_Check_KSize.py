...
def write_ngrams(sequence,kmerlength):
    '''
...
###############################################################
def Running_ML(i,PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
    kmerlength = i
    print(kmerlength)
   ...
   ##############################################################
    Apply_ngrams  = lambda x: write_ngrams(x,kmerlength)
...
def worker(x,index,i,*args):
    ...
    #for re in range(3):
    PeakLabel,PeakSeq = Bringfiles(DataPath+"ARFSimulated_bin"+str(125)+"_Peak.fa")
    ...
    for re in range(3):
        ...
        ACC_list  = Running_ML(i,PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,0)
    ...
    Line1 = str(i)+"-mer\t"+WriteMeanSD(Acc_re)+"\t"+WriteMeanSD(FPR_re)+"\t"+WriteMeanSD(FNR_re)+"\n"
...
if __name__ == '__main__':
    ...
    manager2 = Manager()
    CutList = [5,6,7,8,9] #It's kemr list not CutList
    #CutList = [10]
