...
def Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
    ...
    LabelAll = NonPeakLabel+BorderLabel+PeakLabel 
   ...
   ##############################################################
    ...
    False_Positives = []
    False_Negatives = []
    print(float(LR_hold_TFIDF_pred[1]))
    print(X_TFIDF_test)
    for i in range(0,len(LR_hold_TFIDF_pred)):
        if ((float(LR_hold_TFIDF_pred[i]) == float(0)) & (float(Y_test[i])==float(1))):
            False_Negatives.append(Label_test[i])
        elif ((float(LR_hold_TFIDF_pred[i]) == float(1)) & (float(Y_test[i]) == float(0))):
            False_Positives.append(Label_test[i])
