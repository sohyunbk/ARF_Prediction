...
def Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
   ...
   ##############################################################
    ...
    outfile = open("ARF"+nARF+"_PredictionInfo","w")
    ...
    for train, test in skf.split(XfitDf, Y_Total):
        ...
        #print(X_Total[b])
        #print(Y_test[count])
        #print(LR_hold_TFIDF_pred[count])
            outfile.write(Label_Total[b]+"\t"+ X_Total[b]+"\t"+str(Y_test[count])+"\t"+str(LR_hold_TFIDF_pred[count])+"\n")
            count+=1
