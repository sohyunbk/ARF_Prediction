...
def make_stopwords(kmersize):
    ...
    for n in kmerall:
        ...
        if compute_kmer_entropy(kmer) < limit_entropy:
            ...
            outfile.write(kmer+"\t"+str(compute_kmer_entropy(kmer))+"\n")
        else:
            continue
...
def Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
   ...
   ##############################################################
    ...
    #print(stats['var'])
    fsel = VarianceThreshold(threshold=0.001)
    ...
    #XfitDf
    X_TFIDF_DEV, X_TFIDF_test, Y_train, Y_test = train_test_split(XfitDf, Y_Total, test_size=0.2)
