...
def Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
   ...
   ##############################################################
    ...
    for train, test in skf.split(XfitDf, Y_Total):
        ...
        Y_train_NonPeak = Y_Total[Index_zero]
        ...
        ####################################
        TFIDF_LR = LogisticRegression(C=1.0, class_weight={0:2, 1:1}, dual=False, fit_intercept=True,intercept_scaling=1, max_iter=100, multi_class='ovr', n_jobs=1, penalty='l2', random_state=None, solver='liblinear', tol=0.0001,verbose=0, warm_start=False)
...
def Main(nARF):
    ...
    for x in range(0,3):
        ...
        ACC,FPR,FNR = Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,0)
        ACC_re.append(ACC)
        FPR_re.append(FPR)
        FNR_re.append(FNR)
    print(ACC_re)
    outfile = open("ClassW1_ARF"+str(nARF)+"_5FC_3Re.txt","w")
    #outfile = open("ARF"+str(nARF)+"_5FC_3Re.txt","w")
    outfile.write("ARF"+str(nARF)+"\t"+WriteMeanSD(ACC_re)+"\t"+WriteMeanSD(FPR_re)+"\t"+WriteMeanSD(FNR_re)+"\n")
    outfile.close()
