...
def Running_ML(X_Total_Up, Y_Total_Up,X_TFIDF_test,Y_test,outfile):
    ...
    outfile.write("Evaluating model")
    outfile.write(metrics.classification_report(Y_test, LR_hold_TFIDF_pred)) #y_true, y_pred
    outfile.write("Contigency_matrix from Sohyun")
    outfile.write(metrics.confusion_matrix(Y_test, LR_hold_TFIDF_pred,labels=[0,1,2])) #y_true, y_pred
    outfile.write(metrics.classification_report(Y_test, LR_hold_TFIDF_pred)) #y_true, y_pred
    outfile.write("Accuracy")
    outfile.write(accuracy_score(Y_test, LR_hold_TFIDF_pred))
    outfile.write("Precision")
    outfile.write(precision_score(Y_test, LR_hold_TFIDF_pred))
    outfile.write("Sensitivity")
    outfile.write(recall_score(Y_test, LR_hold_TFIDF_pred))
#########################################################################################

def Main(nARF):
    outfile = open("ARF"+nARF+".txt","w")
    print(nARF)
    PeakLabel_m,PeakSeq_m = Bringfiles(DataPath+"ARF"+nARF+"_bin125_Peak.fa")
    outfile.write("Peak: "+str(len(PeakLabel_m)))
    NonPeakLabel_m,NonPeakSeq_m = Bringfiles(DataPath+"ARF"+nARF+"_bin125_NonPeak.fa")
    outfile.write("Non Peak: "+str(len(NonPeakLabel_m)))
    BorderLabel_m,BorderSeq_m = Bringfiles(DataPath+"ARF"+nARF+"_bin125_Border.fa")
    outfile.write("Non Peak: "+str(len(BorderLabel_m)))

    X_Total_Up, Y_Total_Up = Load_ML_Up(PeakLabel_m,PeakSeq_m,BorderLabel_m,BorderSeq_m,NonPeakLabel_m,NonPeakSeq_m,0)

    PeakLabel_v,PeakSeq_v = Bringfiles(DataPath_v+"ZmARF"+nARF+"_Soybean_bin100_Peak.fa")
    outfile.write("Peak: "+str(len(PeakLabel_v)))
    NonPeakLabel_v,NonPeakSeq_v = Bringfiles(DataPath_v+"ZmARF"+nARF+"_Soybean_bin100_NonPeak.fa")
    outfile.write("Non Peak: "+str(len(NonPeakLabel_v)))
    BorderLabel_v,BorderSeq_v = Bringfiles(DataPath_v+"ZmARF"+nARF+"_Soybean_bin100_Border.fa")
    outfile.write("Non Peak: "+str(len(BorderLabel_v)))

    X_test, Y_test = Load_ML(PeakLabel_v,PeakSeq_v,BorderLabel_v,BorderSeq_v,NonPeakLabel_v,NonPeakSeq_v,0,X_Total_Up)
    Running_ML(X_Total_Up, Y_Total_Up,X_test,Y_test,outfile)
    outfile.close()
...
DataPath_v = "/scratch/sb14489/1-2.ML_NewTry/1-2.MakeFASTA_Soybean/2.OutputFile/"
nARF = str(sys.argv[1])
Main(nARF)
