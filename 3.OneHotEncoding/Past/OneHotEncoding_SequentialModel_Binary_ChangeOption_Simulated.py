...
def Model(X,y,train,test,ACC_list,manager,*args):
    X_train = X[train]
    X_test = X[test]
    y_train = y[train]
    y_test =  y[test]
...
def Main(nARF): 
    ...
    for m in range(nCross):
        train = Train_list[m]
        test = Train_list[m]
        p.append(Process(target=Model, args=(X,y,train,test,ACC_list,manager)))    
...
if __name__ == '__main__':
    ...
    manager2 = Manager()
    nRepeat=2
    x = manager2.list([[]]*nRepeat)
    pro = []
    for i in range(nRepeat):
        pro.append(Process(target=worker, args=(x, i)))
        pro[i].start()
    for i in range(nRepeat):
        pro[i].join()
    ...
    ACC_list, FPR_list, FNR_list = SeparateList(x)
