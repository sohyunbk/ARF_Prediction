...
def Main(nARF):
    print(nARF)
    PeakLabel,PeakSeq = Bringfiles(DataPath+"ZmARF"+nARF+"_Soybean_bin125_Peak.fa")
    print("Peak: "+str(len(PeakLabel)))
    NonPeakLabel,NonPeakSeq = Bringfiles(DataPath+"ZmARF"+nARF+"_Soybean_bin125_NonPeakRemoveR.fa")
    print("Non Peak: "+str(len(NonPeakLabel)))
    BorderLabel,BorderSeq = Bringfiles(DataPath+"ZmARF"+nARF+"_Soybean_bin125_Border.fa")
    ...
    for x in range(0,3):
        ...
        FPR_re.append(FPR)
        FNR_re.append(FNR)
    outfile = open(str(nARF)+"_5FC_3Re.txt","w")
...
DataPath = "/scratch/sb14489/1.ML_ARF/2.ML/1-3.MakeFASTA_Soybean_Redo/2.OutputFile/"
