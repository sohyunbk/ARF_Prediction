...
def Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
   ...
   ##############################################################
    ...
    for train, test in skf.split(XfitDf, Y_Total):
        ...
        #print(ACC)
        top = np.argsort(TFIDF_LR.coef_[0])[-5:]
        #print(" ".join([ i.split('n')[0].upper() for i in feature_names[top] ]))
        Coefficient.append(TFIDF_LR.coef_[0])
        
        #print("Check number here")
        #print(len(TFIDF_LR.coef_[0])) 
        #print(len(colnames))
        #print(top)
        botton = np.argsort(TFIDF_LR.coef_[0])[:5]
        #print(" ".join([ i.split('n')[0].upper() for i in feature_names[botton] ]))
        ACC_FC.append(ACC)
        FPR_FC.append(FPR)
        FNR_FC.append(FNR)
    ...
    return ACC, FPR, FNR, Coefficient,colnames
...
def Main(nARF):
    ...
    ACC,FPR,FNR,Coefficient,feature_names  = Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,0)
    #print(Coefficient)
    #print(Coefficient[4][0])

    nFold = 5
    outfile = open("ARF"+str(nARF)+"_FeatureScore.txt","w")
    #print(len(feature_names))
    for i in range(len(feature_names)):
        sFeature = feature_names[i].upper().replace("N","/")
        Temp = []
        for j in range(nFold):
            Temp.append(Coefficient[j][i])
        Average = sum(Temp)/len(Temp)
        outfile.write(sFeature+"\t"+str(Average)+"\n")
...
DataPath = "/scratch/sb14489/1.ML/1-1.MakeFASTA_Maize/2.Output/"
