...
def Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,nBorder_Choice):
   ...
   ##############################################################
    ...
    return ACC_list,FPR_list,FNR_list

def truncate(num, n):
    integer = int(num * (10**n))/(10**n)
    return float(integer)

def WriteMeanSD(List):
    Average = truncate(statistics.mean(List)*100,2)
    Sd = truncate(statistics.stdev(List)*100,2)
...
Outfile = open("Result_ACC_FPR_FNR.txt","w")
...
for i in CutList:
    ...
    BorderLabel,BorderSeq = Bringfiles(DataPath+"ARFSimulated_bin"+str(i)+"_Border_HalfNumbPeak.fa")	
    ...
    ACC_list,FPR_list,FNR_list = Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,0)
    Outfile.write("Bin"+str(i)+"\tConsideredAsNonPeak\t"+WriteMeanSD(ACC_list)+"\t"+WriteMeanSD(FPR_list)+"\t"+WriteMeanSD(FNR_list)+"\n")
    ### 2) Consider border as peak


    ACC_list,FPR_list,FNR_list = Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,1)
    Outfile.write("Bin"+str(i)+"\tConsideredAsPeak\t"+WriteMeanSD(ACC_list)+"\t"+WriteMeanSD(FPR_list)+"\t"+WriteMeanSD(FNR_list)+"\n")
