...
def worker(x,index,i,*args):
    ...
    Line1 = "Bin"+str(i)+"\tConsideredAsNonPeak\t"+WriteMeanSD(ACC_list)+"\t"+WriteMeanSD(FPR_list)+"\t"+WriteMeanSD(FNR_list)+"\n"
    ### 2) Consider border as peak
    ACC_list  = Running_ML(PeakLabel,PeakSeq,BorderLabel,BorderSeq,NonPeakLabel,NonPeakSeq,1)
    ACC_list, FPR_list, FNR_list = SeparateList(ACC_list)
    Line2 = "Bin"+str(i)+"\tConsideredAsPeak\t"+WriteMeanSD(ACC_list)+"\t"+WriteMeanSD(FPR_list)+"\t"+WriteMeanSD(FNR_list)+"\n"
    Line3 =Line1+Line2
