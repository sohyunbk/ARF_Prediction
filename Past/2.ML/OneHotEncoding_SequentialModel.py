...
def Main(nARF): 
    ...
    model = Sequential()
    conv_layer = Conv1D(filters=320,kernel_size=26,padding='valid',activation='relu')
    model.add(conv_layer)
    #model.add(layers.Conv1D(32,3, activation='relu'))
    #model.Dense(32, activation='relu')
    model.add(MaxPooling1D(input_shape = X_train.shape[1:3],pool_size=13,strides=2,padding='valid'))
    model.add(Flatten())
