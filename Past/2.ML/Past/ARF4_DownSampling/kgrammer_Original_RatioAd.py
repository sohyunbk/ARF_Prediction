...
def CountNumber_Split_Training_Test_DownSampling(Seq,TrainingRatio,nNon,nPeak):
    ...
    nNumberOfInput = len(Seq)
    #######****************************************
    RandomNumberforTrain_NonPeak = random.sample(list(range(0,nNon)), int(nNon*TrainingRatio))
    RandomNumberforTrain_NonPeak_Sub = random.sample(RandomNumberforTrain_NonPeak,int(nPeak*TrainingRatio))
    RandomNumberforTrain_Peak = random.sample(list(range(nNon,nNon+nPeak)), int(nPeak*TrainingRatio))
...
'''
Train = ["ATGGGCCCGC","ATGGGGGTCGC","ATGGGCCCGT"]
Test  = lambda x: write_ngrams(x)
New = list(map(Test, Train))
print(all_tokens)'''

DataPath = "/scratch/sb14489/1.ML_DAPSeq/13.AllCombi/1.MakeFASTA/2.Output/"

#PeakLabel,PeakSeq = Bringfiles(DataPath+"Part_Peak.fa")
#NonPeakLabel,NonPeakSeq = Bringfiles(DataPath+"Part_NonPeak.fa")
#BorderLabel,BorderSeq = Bringfiles(DataPath+"Part_Border.fa")

PeakLabel,PeakSeq = Bringfiles(DataPath+"ARF4_bin125_Peak.fa")
NonPeakLabel,NonPeakSeq = Bringfiles(DataPath+"ARF4_bin125_NonPeakRemoveR.fa")
#BorderLabel,BorderSeq = Bringfiles(DataPath+"ARF4_bin125_Border.fa")

## Combine Different classes!! Test and Tain set should be distinguished
print("ChangeOrder1")
#Label_Total = np.concatenate([NonPeakLabel,PeakLabel,BorderLabel]) 
print("2")
...
#TFIDF_LR = LogisticRegression(C=1.0, class_weight=None, dual=False, fit_intercept=True,intercept_scaling=1, max_iter=100, multi_class='ovr', n_jobs=1,
