...
def Main(nARF): 
    ...
    ##### Resampling ##########
    arr_matrix2_Re = resample(arr_matrix2,replace=True,n_samples=arr_matrix1.shape[0],random_state=123)
    print("X:",arr_matrix1.shape)
    print("X_Re:",arr_matrix2_Re.shape)

    X = np.concatenate((arr_matrix1,arr_matrix2_Re))

    nPeak = len(PeakDic.keys())
    nNonpeak = len(PeakDic.keys())
    print(nPeak)
    ...
    X_train, X_test_notUse, y_train, y_test_notUse = train_test_split( X, y, test_size=0.3)
    ...
    y = np.array(Label_Sample(nPeak,nNonpeak))
    X_train_notUse, X_test, y_train_notUse, y_test = train_test_split( X, y, test_size=0.3)
